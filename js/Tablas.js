const mostrartablasydesplazamietosbinarios = () => {
  ...
  if (numero == 7) {
    ...
    for (let i = 0; i <= 10; i++) {
      tablasumaocho += `<li>${numero} + ${i} = ${
        parseInt(numero) + parseInt(i)
      }</li>`;
    }
    ...
    let Desplazamientobinario = document.getElementById(
      "Desplazamientobinario"
    );

    let Desplazamiento = `<h2>Desplazamiento binario a la derecha del numero  ${numero}: ${
      numero >> 4
    } </h2>`;
    ...
    let Desplazamiento = `<h2>Desplazamiento binario a la izquierda del numero  ${numero} : ${
      numero << 2
    }</h2>`;
